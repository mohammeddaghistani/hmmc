...
def render_admin_panel(user_role):
    ...
    with st.form("settings_form"):
        ...
        if st.form_submit_button("💾 حفظ الإعدادات"):
            ...
            update_setting('construction_cost_m2', cost)import streamlit as st
from modules.db import get_setting, update_setting

def render_admin_panel(user_role):
    # التحقق من الصلاحيات (اختياري حسب منطق نظامك)
    if user_role != "admin":
        st.error("ليس لديك صلاحية للدخول إلى هذه الصفحة")
        return
    ...
    with st.form("settings_form"):
        ...
        # جلب القيم الحالية من قاعدة البيانات أو وضع قيم افتراضية
        m_temp = st.number_input("معامل التأجير المؤقت", 
                                 value=float(get_setting('mult_temp', 0.85)), 
                                 step=0.01)
        
        m_long = st.number_input("معامل الاستثمار طويل الأجل", 
                                 value=float(get_setting('mult_long', 1.60)), 
                                 step=0.01)
        
        cost = st.number_input("تكلفة البناء المعتمدة (ر/م²)", 
                               value=float(get_setting('construction_cost_m2', 3500)), 
                               step=100.0)
        ...
        if submit:
            ...
            # إعادة تحميل الصفحة لتحديث القيم في الذاكرة إذا لزم الأمر
            st.rerun()
            st.success("✅ تم تحديث المعدلات بنجاح")
