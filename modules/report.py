import plotly.express as px
import streamlit as st
import pandas as pd
from datetime import datetime
from fpdf import FPDF
import base64
from io import BytesIO
import arabic_reshaper
from bidi.algorithm import get_display

# --- دالة مساعدة لمعالجة النصوص العربية ---
def fix_arabic(text):
    if not text:
        return ""
    # إعادة تشكيل الحروف ثم عكس الاتجاه (من اليمين لليسار)
    reshaped_text = arabic_reshaper.reshape(text)
    return get_display(reshaped_text)

class PDFReport(FPDF):
    """فئة مخصصة لتوليد تقارير PDF تدعم العربية مع إدارة المساحات"""
    
    def header(self):
        # إضافة شعار (تأكد من وجود المجلد والصورة)
        try:
            self.image('assets/logo.png', 10, 8, 33)
        except:
            pass 
            
        # إضافة خط يدعم العربية
        try:
            self.add_font('DejaVu', '', 'fonts/DejaVuSans.ttf')
            self.set_font('DejaVu', '', 16)
        except:
            self.set_font('Arial', 'B', 16)
            
        title = fix_arabic('تقرير التقييم العقاري')
        self.cell(0, 10, title, 0, 0, 'C')
        self.ln(20)
    
    def footer(self):
        self.set_y(-15)
        try:
            self.add_font('DejaVu', '', 'fonts/DejaVuSans.ttf')
            self.set_font('DejaVu', '', 8)
        except:
            self.set_font('Arial', 'I', 8)
            
        page_num = fix_arabic(f'الصفحة {self.page_no()}')
        self.cell(0, 10, page_num, 0, 0, 'C')
    
    def add_arabic_content(self, text):
        """إضافة نص عربي مع حل مشكلة المساحة الأفقية (Horizontal Space)"""
        try:
            self.add_font('DejaVu', '', 'fonts/DejaVuSans.ttf')
            self.set_font('DejaVu', '', 12)
        except:
            st.error("خطأ: ملف الخط غير موجود في fonts/DejaVuSans.ttf")
            return
            
        # ضبط الهوامش لضمان توفر مساحة كافية للرسم
        self.set_right_margin(15)
        self.set_left_margin(15)
        
        # معالجة النص أسطر بأسطر لتجنب أخطاء المساحة
        for line in text.split('\n'):
            if not line.strip():
                self.ln(5)
                continue
            
            processed_line = fix_arabic(line)
            # استخدام w=0 يخبر المكتبة باستخدام كامل العرض المتاح تلقائياً
            self.multi_cell(w=0, h=10, txt=processed_line, align='R')

def render_report_module(user_role):
    """عرض وحدة التقارير بجميع خصائصها"""
    st.markdown("""
    <div style="text-align: right; direction: rtl;">
        <h2>📑 نظام التقارير والإحصائيات</h2>
        <p>توليد تقارير احترافية وتحليلات مفصلة</p>
    </div>
    """, unsafe_allow_html=True)
    
    # تبويبات التقارير
    tab1, tab2, tab3, tab4 = st.tabs(["📊 تقارير التقييم", "📈 إحصائيات", "🎯 تقارير مخصصة", "📤 تصدير البيانات"])
    
    with tab1:
        render_evaluation_reports()
    
    with tab2:
        render_statistics()
    
    with tab3:
        render_custom_reports()
    
    with tab4:
        render_export_options()

def render_evaluation_reports():
    """عرض تقارير التقييم مع الفلاتر والأزرار"""
    st.subheader("📋 تقارير التقييمات المكتملة")
    
    col1, col2, col3 = st.columns(3)
    with col1:
        st.selectbox("نوع التقرير", ["جميع التقارير", "معلقة", "مكتملة", "ملغاة"])
    with col2:
        st.date_input("من تاريخ")
    with col3:
        st.date_input("إلى تاريخ")
    
    reports = [
        {"id": "REP-2024-001", "property": "حي النخيل - الرياض", "value": "450,000 ر.س", "confidence": "92%", "status": "مكتمل", "date": "2024-01-15", "prepared_by": "المقيّم أحمد"},
        {"id": "REP-2024-002", "property": "حي الياسمين - جدة", "value": "320,000 ر.س", "confidence": "85%", "status": "معلق", "date": "2024-01-14", "prepared_by": "المقيّم محمد"},
        {"id": "REP-2024-003", "property": "حي الربيع - الدمام", "value": "380,000 ر.س", "confidence": "88%", "status": "مكتمل", "date": "2024-01-13", "prepared_by": "المقيّم خالد"}
    ]
    
    for report in reports:
        with st.expander(f"📄 {report['id']} - {report['property']}"):
            c1, c2, c3 = st.columns(3)
            c1.metric("💰 القيمة", report['value'])
            c2.metric("⭐ الثقة", report['confidence'])
            status_color = "🟢" if report['status'] == "مكتمل" else "🟡"
            c3.metric("📊 الحالة", f"{status_color} {report['status']}")
            
            st.caption(f"📅 التاريخ: {report['date']} | 👤 المعد: {report['prepared_by']}")
            
            b1, b2, b3 = st.columns(3)
            with b1:
                if st.button("📄 عرض التقرير", key=f"view_{report['id']}"):
                    generate_pdf_report(report)
            with b2:
                if st.button("📧 إرسال", key=f"send_{report['id']}"):
                    st.success(f"تم إرسال التقرير {report['id']}")
            with b3:
                if st.button("🗑️ حذف", key=f"delete_{report['id']}", type="secondary"):
                    st.warning(f"هل أنت متأكد من حذف التقرير {report['id']}?")

def generate_pdf_report(report_data):
    """توليد تقرير PDF مع إصلاح خطأ الترميز"""
    pdf = PDFReport()
    pdf.set_auto_page_break(auto=True, margin=15)
    pdf.add_page()
    
    content = f"""
تقرير التقييم العقاري
=====================

معلومات التقرير:
رقم التقرير: {report_data['id']}
العقار: {report_data['property']}
القيمة المقدرة: {report_data['value']}
درجة الثقة: {report_data['confidence']}
الحالة: {report_data['status']}
التاريخ: {report_data['date']}
المعد: {report_data['prepared_by']}

ملخص التقييم:
تم إجراء التقييم بناءً على منهجية علمية تعتمد على دراسة السوق العقاري المحلي وتطبيق المعايير الاحترافية.

التوصيات:
• القيمة المقدرة تعكس السوق الحالي.
• درجة الثقة {report_data['confidence']} تشير إلى موثوقية عالية.

التوقيع:
_________________________
مدير التقييم - نظام التقييم الإيجاري
    """
    
    pdf.add_arabic_content(content)
    
    # الحل الجذري لـ latin-1: مكتبة fpdf2 تعيد bytes مباشرة عبر output()
    pdf_bytes = pdf.output()
    
    b64 = base64.b64encode(pdf_bytes).decode()
    href = f'''
    <div style="text-align: center; margin-top: 10px;">
        <a href="data:application/pdf;base64,{b64}" download="تقرير_تقييم_{report_data['id']}.pdf" 
           style="background-color: #4CAF50; color: white; padding: 10px 20px; text-decoration: none; border-radius: 5px; font-weight: bold;">
           📥 انقر هنا لتحميل التقرير (PDF)
        </a>
    </div>
    '''
    st.markdown(href, unsafe_allow_html=True)
    st.success("تم تجهيز التقرير بنجاح.")

def render_statistics():
    """عرض الإحصائيات باستخدام Plotly"""
    st.subheader("📈 إحصائيات وأداء النظام")
    col1, col2 = st.columns(2)
    
    with col1:
        st.markdown("<h4>📊 توزيع التقييمات حسب النوع</h4>", unsafe_allow_html=True)
        df = pd.DataFrame({'النوع': ['سكني', 'تجاري', 'مكتبي', 'صناعي', 'أخرى'], 'النسبة': [45, 25, 15, 10, 5]})
        fig = px.pie(df, values='النسبة', names='النوع', color_discrete_sequence=px.colors.sequential.Blues)
        fig.update_layout(showlegend=True, height=300)
        st.plotly_chart(fig, use_container_width=True)
    
    with col2:
        st.markdown("<h4>📅 التقييمات خلال آخر 6 أشهر</h4>", unsafe_allow_html=True)
        fig = px.bar(x=['يوليو', 'أغسطس', 'سبتمبر', 'أكتوبر', 'نوفمبر', 'ديسمبر'], y=[120, 135, 150, 165, 180, 195],
                    labels={'x': 'الشهر', 'y': 'عدد التقييمات'}, color_continuous_scale='Blues')
        fig.update_layout(height=300, showlegend=False)
        st.plotly_chart(fig, use_container_width=True)
    
    st.markdown("---")
    c3, c4, c5, c6 = st.columns(4)
    for col, (t, v, ch) in zip([c3, c4, c5, c6], [("🏢 إجمالي التقييمات", "1,245", "+12%"), ("⭐ متوسط الثقة", "87%", "+5%"), ("💰 متوسط القيمة", "425K", "+3%"), ("⏱️ متوسط وقت التقييم", "2.5 ساعة", "-15%")]):
        with col: st.metric(t, v, ch)

def render_custom_reports():
    """عرض التقارير المخصصة مع النماذج"""
    st.subheader("🎯 تقارير مخصصة حسب المعايير")
    with st.form("custom_report_form"):
        col1, col2 = st.columns(2)
        with col1:
            report_period = st.selectbox("الفترة الزمنية", ["الأسبوع الحالي", "الشهر الحالي", "الربع الحالي", "السنة الحالية", "مخصص"])
            property_types = st.multiselect("أنواع العقارات", ["سكني", "تجاري", "مكتبي", "صناعي", "زراعي"], default=["سكني", "تجاري"])
        with col2:
            cities = st.multiselect("المدن", ["الرياض", "جدة", "الدمام", "مكة", "المدينة", "الشرقية"], default=["الرياض", "جدة"])
            min_confidence = st.slider("أقل درجة ثقة", 0, 100, 70)
        
        st.markdown("---")
        c3, c4 = st.columns(2)
        with c3: output_format = st.radio("صيغة الملف", ["PDF", "Excel", "CSV", "HTML"])
        with c4: 
            charts = st.checkbox("📊 تضمين الرسوم البيانية", value=True)
            details = st.checkbox("📋 تضمين التفاصيل الكاملة", value=True)
            
        if st.form_submit_button("🚀 توليد التقرير", use_container_width=True):
            st.success("✅ تم توليد التقرير المخصص بنجاح!")
            st.dataframe(pd.DataFrame({"المعيار": ["الفترة", "أنواع العقارات", "المدن"], "القيمة": [report_period, ", ".join(property_types), ", ".join(cities)]}), use_container_width=True)

def render_export_options():
    """عرض خيارات التصدير والجدولة"""
    st.subheader("📤 تصدير البيانات والإحصائيات")
    col1, col2 = st.columns(2)
    with col1:
        st.info("📊 تصدير البيانات الخام (Excel/CSV)")
        st.button("تصدير الآن", key="export_raw_btn")
    with col2:
        st.info("📈 تصدير تقارير الأداء (PDF/PPT)")
        st.button("تصدير الآن", key="export_stats_btn")
    
    st.markdown("---")
    st.subheader("🕐 تصدير مجدول")
    c3, c4 = st.columns(2)
    with c3: st.selectbox("نوع الجدولة", ["يومي", "أسبوعي", "شهري"])
    with c4: st.time_input("وقت التصدير")
    st.text_input("المستلمون (البريد الإلكتروني)")
    st.button("✅ تفعيل الجدولة", use_container_width=True)
